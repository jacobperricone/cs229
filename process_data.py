...
file_types = ['aggregatematchups', 'playerstatsbyteam','teamstats']
...
def write_to_db(engine, frame, table_name, chunk_size, if_exist_param = 'replace'):
    for idx, chunk in enumerate(chunks(frame, chunk_size)):
        chunk.to_sql(con=engine, name=table_name, if_exists=if_exist_param)

#fill database directory with tables
def fill_database(directory):

    engine = create_engine('mysql+mysqlconnector://root:Redstring01!1@127.0.0.1:3306/' + directory,
                                   echo=False)
        ...
        for i, file_name in enumerate(files):
            ...
            try:
                ...
                if i == 0:
                    ...
                    x = x[headers]
                    write_to_db(engine, x, table_type, 5000, 'append')
...
def main():
    ...
    # for dir in master_directories: fill_database(dir)
